...
export default async (client, guild) => {
    ...
    client.log(
        ...
        client.intlGet(null, 'fcmListenerStartHost', {
            guildId: guild.id,
            steamId: hoster,
        }),
    );
    ...
    client.fcmListeners[guild.id] = await PushReceiver.listen(
        credentials[hoster].fcm_credentials,
                ...
                default:
                    {
                        client.log(
                            'FCM Host',
                            `GuildID: ${guild.id}, SteamID: ${hoster}, other\n${JSON.stringify(full)}`,
                        );
                    }
                    break;
            }
...
async function alarmAlarm(client, guild, full, data, body) {
    ...
    if (
        (!rustplus || (rustplus && rustplus.serverId !== serverId)) &&
...
async function alarmRaidAlarm(client, guild, full, data, body) {
    ...
    const files = [];
    if (body.img === '') {
        // @ts-expect-error TS(2345) FIXME: Argument of type 'AttachmentBuilder' is not assign... Remove this comment to see the full error message
