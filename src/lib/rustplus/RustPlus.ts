...
export class RustPlus extends EventEmitter {
    ...
    connect(): void {
        ...
        this.websocket.on('message', (data: any) => {
            ...
            this.emit('message', rustplus.AppMessage.decode(data));
        });
    ...
    sendRequest(data: any, callback?: (message: any) => boolean | void): void {
        ...
        const request = rustplus.AppRequest.fromObject({
            seq: currentSeq,
        ...
        this.websocket!.send(rustplus.AppRequest.encode(request).finish());
